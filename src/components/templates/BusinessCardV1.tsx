...
const BusinessCardV1 = ({ data }: { data?: any }) => {
  const { t } = useI18n();
  const name = data?.name || "Claudia Alves";
  const role = data?.role || "Business Marketing";
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="mt-12 p-[2px] rounded-3xl shadow-[0_0_20px_rgba(212,175,55,0.3)]"
        style={{ background: data?.theme?.gradient || 'linear-gradient(to top right, #C5A028, #F3E5AB, #C5A028)' }}
      >
        <div className="rounded-3xl overflow-hidden bg-black w-44 h-44 flex items-center justify-center">
          {avatarUrl ? (
            <img
              src={avatarUrl}
              alt={name}
      ...
      {/* Social Icons */}
      <div className="flex space-x-3 mt-8 rtl:space-x-reverse">
        ...
        <WebIcon href={website?.startsWith('http') ? website : `https://${website}`} />
      </div>

      {/* Contact Information */}
      <div className="text-gray-300 text-left w-full mt-10 space-y-3 px-8 text-sm font-light flex-grow">
        <div className="flex items-center"><span className="w-24 font-medium text-gray-400">{t('id_card')}</span><span className="text-white">: 2026777</span></div>
        <motion.a whileHover={{ x: 4 }} href={`mailto:${email}`} className="flex items-center group cursor-pointer block hover:text-white transition-colors"><span className="w-24 font-medium text-gray-400 group-hover:text-gray-300">{t('email')}</span><span className="text-white truncate">: {email}</span></motion.a>
        <motion.a whileHover={{ x: 4 }} href={`tel:${phone}`} className="flex items-center group cursor-pointer block hover:text-white transition-colors"><span className="w-24 font-medium text-gray-400 group-hover:text-gray-300">{t('phone')}</span><span className="text-white">: {phone}</span></motion.a>
        <motion.a whileHover={{ x: 4 }} href={website?.startsWith('http') ? website : `https://${website}`} target="_blank" rel="noopener noreferrer" className="flex items-center group cursor-pointer block hover:text-white transition-colors"><span className="w-24 font-medium text-gray-400 group-hover:text-gray-300">{t('website')}</span><span className="text-white truncate">: {website}</span></motion.a>
        <motion.a whileHover={{ x: 4 }} href={`https://maps.google.com/?q=${encodeURIComponent(data?.location || '75008 Paris, France')}`} target="_blank" rel="noopener noreferrer" className="flex items-center group cursor-pointer block hover:text-white transition-colors"><span className="w-24 font-medium text-gray-400 group-hover:text-gray-300">{t('address')}</span><span className="text-white truncate">: {data?.location || '75008 Paris, France'}</span></motion.a>
      ...
      {/* QR Code */}
      <div className="mt-auto mb-8 bg-white/5 p-3 rounded-2xl backdrop-blur-md border border-white/10">
        <div className="bg-white p-2 rounded-xl">
          <QRCodeSVG value={website || "https://example.com"} size={100} />
        </div>
      </div>
    </motion.div>
  );
