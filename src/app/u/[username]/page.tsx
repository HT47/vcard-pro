...
export default function PublicProfilePage() {
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      navigator.share({
        ...
        text: `Découvrez la vCard de @${username}`,
        url,
      }).catch(() => {});
    } else {
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-[#050505] text-white">
        <div className="text-center p-8 bg-black/50 border border-white/10 rounded-2xl backdrop-blur-xl max-w-sm">
          ...
          <p className="text-zinc-400 text-sm mb-6">@{username} n'existe pas ou a supprimé son profil.</p>
  ...
  return (
    <I18nProvider>
      <div className={`min-h-screen w-full ${isLight ? "bg-[#f0f2f5]" : "bg-[#050505]"}`}>
        ...
        <div className="max-w-md mx-auto px-4 -mt-16 relative z-10 pb-24">
          ...
          <div className={`rounded-3xl border shadow-2xl overflow-hidden ${isLight ? "bg-white border-zinc-100" : "bg-zinc-950 border-white/10"}`}>
            ...
            <div className="px-5 pb-5 grid grid-cols-2 gap-3">
              ...
              >
                {addedToContacts ? (
                  <><CheckCircle2 size={18} /> Contact ajouté !</>
                ) : (
                  <><Save size={18} /> Ajouter aux contacts</>
                )}
              </motion.button>
              <motion.button
                whileHover={{ scale: 1.02 }}
            ...
            <AnimatePresence mode="wait">
              ...
              {activeView === "contact" && (
                ...
                >
                  {[
                    { icon: Mail, label: "Email", value: vcardData?.email, href: `mailto:${vcardData?.email}` },
                    { icon: Phone, label: "Téléphone", value: vcardData?.phone, href: `tel:${vcardData?.phone}` },
                    { icon: Globe, label: "Site Web", value: vcardData?.website, href: vcardData?.website },
                    { icon: MapPin, label: "Adresse", value: vcardData?.location, href: `https://maps.google.com/?q=${encodeURIComponent(vcardData?.location || "")}` },
                  ...
                  {vcardData?.socialLinks && vcardData.socialLinks.filter(l => l.url).length > 0 && (
                    <div className="pt-2">
                      <p className={`text-[10px] uppercase tracking-widest font-bold mb-3 ${isLight ? "text-zinc-400" : "text-zinc-500"}`}>Réseaux</p>
          ...
          >
            ...
            Créé avec <strong className="text-white">VCard Pro</strong> ⚡
          </a>
        </div>
        ...
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 p-1.5 bg-black/70 backdrop-blur-xl border border-white/10 rounded-full shadow-2xl">
          ...
          >
            ...
            {addedToContacts ? "Ajouté !" : "Ajouter aux contacts"}
          </button>
          <button
            onClick={handleShare}
        ...
        <AnimatePresence>
          {showQR && (
            <>
              ...
              >
                <h3 className="text-lg font-bold text-white">Scanner mon profil</h3>
                ...
                <button onClick={() => setShowQR(false)} className="text-zinc-400 text-sm hover:text-white transition-colors">Fermer</button>
              </motion.div>
            </>
          )}
        </AnimatePresence>
